...
def get_layout(actual_category='ELETRO LINHA BRANCA'):
    return html.Div(children=[
                            dcc.Store(id='memory', storage_type='memory', data=get_top_list(app1.data_d, app2.cats[actual_category]).to_json()),
                            html.Div(children=[
                            ...
                            html.Div(children=[
                                html.Div(id='top5-table', children=[
                                    html.P(children="Top 5 produtos de " + actual_category, className="title"), 
                                    draw_top_list(app1.data_d, app2.cats[actual_category])], 
...
def update_chart(active_cell, data):
    if active_cell: # Uma célula foi selecionada
        return get_sales_figure(app1.data_d,  pd.read_json(data).iloc[active_cell['row']]['Produto'])
